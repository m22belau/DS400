...
table(fake_news$title_has_exc)
```

```{r}
#Bar plot: counts of news type
ggplot(fake_news, aes(x = type, fill = type)) +
  geom_bar() +
  theme_bw() +
  labs(title = "Counts of News Types", x = "Type", y = "Count") +
...
prior_probs <- pop_vs_soda %>%
  count(region) %>%             # count number of people in each region
  mutate(probability = n / sum(n))  # divide by total to get P(region)

prior_probs

P_S <- prior_probs$probability[prior_probs$region == "South"]
P_N <- prior_probs$probability[prior_probs$region == "Northeast"]
P_W <- prior_probs$probability[prior_probs$region == "West"]
P_M <- prior_probs$probability[prior_probs$region == "Midwest"]
...
prior_probs %>% select(region, probability)
...
# Calculate likelihoods P(A|region)
likelihoods <- pop_vs_soda %>%
  group_by(region) %>%
  summarise(P_A_given_region = mean(pop == TRUE, na.rm = TRUE))  # proportion using "pop"

likelihoods

# Merge with prior probabilities
likelihoods <- likelihoods %>%
  left_join(prior_probs, by = "region")  # join with prior P(region)

# Total probability P(A)
P_A <- sum(likelihoods$P_A_given_region * likelihoods$probability)
P_A

# Extract P(A|S) and P(S)
P_A_given_S <- likelihoods$P_A_given_region[likelihoods$region == "south"]
P_S <- likelihoods$probability[likelihoods$region == "south"]
...
# 1️⃣ Summarize data by state
pop_by_state <- pop_vs_soda %>%
  group_by(state) %>%
  summarise(percent_pop = mean(pop == TRUE, na.rm = TRUE) * 100) %>%
  mutate(region = tolower(state))  # match map_data

# 2️⃣ Get US map
us_states <- map_data("state")

# 3️⃣ Merge map and data
map_data_pop <- us_states %>%
  left_join(pop_by_state, by = "region")

# 4️⃣ Plot choropleth
ggplot(map_data_pop, aes(x = long, y = lat, group = group, fill = percent_pop)) +
