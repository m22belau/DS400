...
```{r, message=FALSE}
penguins <- read_csv(here("penguins_data", "penguins_lter.csv"))
...
names(penguins)[names(penguins) == "culmen_length_mm"] <- "culmen_length_mm_new"
```
...
```{r}
penguins <- penguins %>% 
  rename(culmen_depth_mm_clean = culmen_depth_mm)
```
...
ggplot(penguins, aes(x = culmen_depth_mm_clean, fill = species)) +
  geom_histogram(position = "identity", alpha = 0.7, bins = 30) +
  theme_bw() +
  labs(
    title = "Distribution of Penguin Culmen Depths",
...
ggplot(penguins, aes(x = culmen_depth_mm_clean, y = body_mass_g, color = species)) +
...
```{r}
scatter_plot <- ggplot(data = penguins, 
...
-   Let's create a new dataframe called *average_body_masses* that has the average body mass for each penguin species. We'll do this by using the *group_by()* and *summarise()* functions.
    ...
    ```{r}
    average_species_body_mass <- penguins %>% 
      group_by(species) %>% 
...
average_species_flipper_length <- penguins %>%
  group_by(species) %>%
  summarise(average_flipper_length = mean(flipper_length_mm, na.rm = TRUE))

# View the result
average_species_flipper_length
...
ggplot(average_species_flipper_length, aes(x = species, y = average_flipper_length, fill = species)) +
  ...
  theme_bw() +
  labs(
    title = "Average Flipper Length by Penguin Species",
    subtitle = "Based on cleaned penguins dataset",
    x = "Species",
...
```{r}
penguins <- bind_rows(gentoo_penguins_data_from_url, adelie_penguins_data_from_url) %>%
  clean_names()
```
...
torgersen_count <- sum(penguins$island == "Torgersen", na.rm = TRUE)
torgersen_count
```
...
average_culmen_depth <- penguins %>%
  group_by(species) %>%
  summarise(average_culmen_depth = mean(culmen_depth_mm_clean, na.rm = TRUE))
