...
model <- lm(rides ~ temp_feel, data = bikes)

# Create prediction point
new_data <- data.frame(temp_feel = 75)
predicted <- predict(model, newdata = new_data, interval = "confidence")
...
ggplot(bikes, aes(x = temp_feel, y = rides)) +
  ...
  annotate("text", x = 75, y = predicted[1], 
           label = paste0("Predicted ≈ ", round(predicted[1])), 
           vjust = -1, color = "red") +
  labs(
    title = "Predicted Number of Rides at 75°F",
